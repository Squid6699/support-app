...
function IncidenciasAdmin() {
    ...
    const { data: incidencias, isLoading: isLoadingIncidencias, refetch: refetchIncidencias } = useQuery<Incidencias[]>({
        queryKey: ["IncidenciasAdmin", filtroEstado],
        queryFn: obtenerIncidenciasAdmin,
    });
    ...
    const [openModalAsignarPrioridad, setOpenModalAsignarPrioridad] = useState(false);
    const [incidenciaAsignarPrioridad, setIncidenciaAsignarPrioridad] = useState<number | null>(null);

    const handleOpenModalAsignarPrioridad = (value: boolean, incidenciaId: number | null) => {
        setOpenModalAsignarPrioridad(value);
        setIncidenciaAsignarPrioridad(incidenciaId);
    };

    const handleCloseModalAsignarPrioridad = () => {
    ...
    async function obtenerIncidenciasAdmin() {
        try {
            let url = HOST + "api/incidenciasAdmin";

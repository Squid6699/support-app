...
function ModalSolicitarPieza({ open, incidencia, handleModalClose, refetchIncidencias }: ModalSolicitarPiezaProps) {
    ...
    async function submitSolicitudPieza(e: React.FormEvent<HTMLFormElement>) {
        ...
        try {
            const response = await fetch(HOST + "api/solicitarPieza", { 
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    "x-frontend-header": "frontend",
                },
                body: JSON.stringify(solicitudPieza),
            });
            ...
            toast.error("OCURRIÓ UN ERROR al solicitar la pieza");
        }
    }
        ...
        <Modal open={open} onClose={handleModalClose}>
            <Box sx={style} component={"form"} onSubmit={submitSolicitudPieza}>
                ...
                <Box sx={{ m: 2 }}>
                    <TextField
                        name="tipo_pieza"
                        label="Tipo de Pieza Solicitada"
                        value={solicitudPieza.tipo_pieza}
                        onChange={handleChange}
                        error={!!solicitudError.tipo_pieza}
                        helperText={solicitudError.tipo_pieza}
                        fullWidth
                        required
                    />
                </Box>
                ...
                <Box sx={{ m: 2, display: 'flex', gap: 1, justifyContent: 'flex-end' }}>
                    <Button onClick={handleCloseModalSolicitud} color="secondary" variant="outlined">
                        Cancelar
                    </Button>

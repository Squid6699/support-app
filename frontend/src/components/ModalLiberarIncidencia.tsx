...
interface ModalLiberarIncidenciaProps {
    open: boolean;
...
function ModalLiberarIncidencia({ open, setOpenModalIncidencia, refetchIncidencias, id }: ModalLiberarIncidenciaProps) {
    ...
    async function submitIncidencia(e: React.FormEvent<HTMLFormElement>) {
        ...
        try {
            const response = await fetch(HOST + "api/liberarIncidencia", {
                method: 'PUT',
                headers: {
                    ...
                    <Box id="modal-modal-description" sx={{ mt: 2 }}>
                        ...
                        <Box sx={{ m: 2, width: '100%' }}>
                            <Typography id="modal-modal-title" component="h2">
                                ¿DESEA LIBERAR LA INCIDENCIA #{id}?
                            </Typography>
                        </Box>
...
export default ModalLiberarIncidencia;